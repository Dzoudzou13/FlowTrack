    ...
    <div class="app-content">
      ...
      <div class="board-page-header">
        ...
        <div class="board-filter-row">
          <a href="<?= htmlspecialchars(app_url('/projects/' . $project['id'] . '/tasks/create'), ENT_QUOTES, 'UTF-8') ?>" class="btn btn-ghost btn-sm">Plný formulár</a>
      ...
      <div class="board-wrap">
        <div class="board-layout">
          ...
          <?php foreach ($columns as $col): ?>
            ...
            <div class="kanban-col" data-status="<?= htmlspecialchars($slug, ENT_QUOTES, 'UTF-8') ?>" data-col-id="<?= (int) $col['id'] ?>">
              <div class="kanban-col-header">
                ...
                <form method="POST" action="<?= htmlspecialchars(app_url('/projects/' . $project['id'] . '/columns/' . $col['id'] . '/delete'), ENT_QUOTES, 'UTF-8') ?>" style="margin:0;" onsubmit="return confirm('Zmazať stĺpec „<?= htmlspecialchars($col['name'], ENT_QUOTES, 'UTF-8') ?>"? Tasky v ňom ostanú.')">
          ...
          <div class="kanban-col kanban-col-add" id="add-col-block">
            ...
            <form method="POST" action="<?= htmlspecialchars(app_url('/projects/' . $project['id'] . '/columns'), ENT_QUOTES, 'UTF-8') ?>" class="add-col-form" id="add-col-form" style="display:none;">
              <input type="text" name="name" placeholder="Názov stĺpca" class="form-control" maxlength="50" required autofocus>
...
<div class="task-drawer" id="task-drawer">
  ...
  <form method="POST" action="<?= htmlspecialchars(app_url('/tasks/quick'), ENT_QUOTES, 'UTF-8') ?>" class="task-drawer-body">
    <input type="hidden" name="_back" value="<?= htmlspecialchars(app_url('/projects/' . $project['id'] . '/board'), ENT_QUOTES, 'UTF-8') ?>">

    <div class="form-group">
      <label class="form-label">Projekt</label>
      <select name="project_id" class="form-control">
        <?php foreach ($projects as $p): ?>
          <option value="<?= (int) $p['id'] ?>" <?= $p['id'] == $project['id'] ? 'selected' : '' ?>>
    ...
    <div class="form-group" id="drawer-col-group">
